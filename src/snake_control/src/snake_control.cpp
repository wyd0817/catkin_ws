...
SShapedLongitudnalWave SnakeControl::s_shaped_longitudnal_wave_(
    /* spec             = */ spec,
...
void SnakeControl::OperateMoveShapedLongitudnalWave(joy_handler_hori::JoySelectedData joy_data)
{
	...
	if(joy_data.joy_stick_l_y_upwards){
		...
		s_shaped_longitudnal_wave_.add_v(0);
	}*/
  ...
  if(joy_data.button_circle){
    s_shaped_longitudnal_wave_.set_flag_on();
  }

  /***  phiを入れるflag_ OFF にする,   A*sech(ω*t )  ***/
  if(joy_data.button_cross){
    s_shaped_longitudnal_wave_.set_flag_off();
  }
	...
	// 	s_shaped_longitudnal_wave_.add_bias(js_data);
	// }else{
